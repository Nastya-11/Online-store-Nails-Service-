...
const DeliveryPostomat: React.FC<Props> = ({
  infoDelivery,
  setInfoDelivery,
  localityId,
  lang,
      ...
      try {
        ...
        if (data.success) {
          const departmentList = data.data
            .filter((x: any) => x.CategoryOfWarehouse == 'Postomat')
            .map((dep: any) => ({
              id: dep.Ref,
          ...
          {!infoDelivery.isTrue &&
            filteredDepartments.map((dep) => (
              <li
                key={dep.id}
                onClick={() => {
                  setInfoDelivery({ res: dep, isTrue: true, text: lang == 'uk' ? dep.name : dep.nameru});
                  setInputValue(lang == 'uk' ? dep.name : dep.nameru);
                  setFilteredDepartments([]);
                }}
              >
                {lang == 'uk' ? dep.name : dep.nameru}
              </li>
            ))}
        </ul>
      )}
...
export default DeliveryPostomat;
